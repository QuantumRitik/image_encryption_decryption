...
def encrypt_image_des(input_image_path, output_image_path, key):
    # Load the image
    ...
    unique_iv = HMAC.new(key_specific, msg=os.urandom(16), digestmod=SHA256).digest()[:8]  # DES uses 8-byte IV
    if len(unique_iv) != 8:
    ...
    cipher = DES.new(key, DES.MODE_CBC, unique_iv)
...
def decrypt_image_des(input_image_path, output_image_path, key, iv_path):
...
if __name__ == "__main__":
    choice = input("Enter 'e' to encrypt or 'd' to decrypt using DES: ").lower()
    key = 'UZ4i59vP'  # DES requires 8-byte (64-bit) keys
    ...
    if choice == 'e':
        ...
        encrypt_image_des(input_image_path, output_image_path, key)
        ...
        decrypt_image_des(input_image_path, output_image_path, key, iv_path)
